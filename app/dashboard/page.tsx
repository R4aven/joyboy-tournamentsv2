...
export default function DashboardPage() {
  ...
  try {
    // useRealtime peut crasher si table notifications manque
  ...
  useEffect(() => {
    const fetchDashboard = async () => {
      ...
      try {
        ...
        const p: any = profile || {};
        const mp = (p.wins ?? p.victories ?? 0) + (p.losses ?? p.defaites ?? 0);
        const wr = mp > 0 ? Math.round(((p.wins ?? 0) / mp) * 100) : 0;
        setStats({
          matchesPlayed: mp,
          wins: p.wins ?? 0,
          losses: p.losses ?? 0,
          winRate: wr,
          tournamentsPlayed: p.tournaments_played ?? p.tournaments ?? 0,
          tournamentsWon: p.tournaments_won ?? 0,
          duelsWon: p.duels_won ?? p.challenges_won ?? 0,
      ...
      <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
        <div>
          ...
          <h1 className="mt-4 text-[30px] lg:text-[38px] font-black tracking-tighter leading-none text-white">
            Bonjour <span className="bg-gradient-to-r from-[#7C3AED] to-[#06B6D4] bg-clip-text text-transparent">{(profile as any)?.username || (profile as any)?.pseudo || (profile as any)?.efootball_pseudo || user?.email?.split("@")[0]}</span> 👋
          </h1>
      ...
      <div className="mt-8 grid grid-cols-1 lg:grid-cols-[1.7fr_1fr] gap-6">
        ...
        <div className="space-y-6">
          ...
          <div className="rounded-[20px] border border-[#22222F] bg-[#101015] p-6">
            ...
            <Link href={`/profile/${(profile as any)?.username || (profile as any)?.pseudo || (profile as any)?.efootball_pseudo || ""}`} className="mt-4 flex h-10 w-full items-center justify-center gap-2 rounded-xl bg-[#15151E] border border-[#22222F] text-[12px] font-bold text-zinc-300 hover:text-white transition-colors">
