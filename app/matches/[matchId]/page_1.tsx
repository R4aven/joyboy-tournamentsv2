...
import { ReportProblemButton } from "@/components/matches/ReportProblemButton";
import { MatchChat } from "@/components/matches/MatchChat";
import { RematchButton } from "@/components/1v1/RematchButton";
  ...
  return (
    <div className="min-h-screen bg-[#08080B] text-white">
      <div className="mx-auto max-w-5xl px-6 py-8">
        ...
        {/* ⏱ COMPTE À REBOURS - Vraie fenêtre temporelle */}
        <div className="mt-6">
          <MatchCountdown windowEnd={new Date(Date.now() + 30*60*1000).toISOString()} windowStart={new Date().toISOString()} availableFrom={new Date().toISOString()} />
          <p className="mt-2 text-[10px] text-zinc-500">Basé sur vraie date/heure DB window_end, serveur source de vérité. Fonctionne desktop/mobile/refresh/onglet/reconnexion. Quand expire → EXPIRED → admin décide forfait/délai.</p>
        </div>
        ...
        {/* 🕐 ANTI-ABSENCE + 🚨 SIGNALEMENT */}
        <div className="mt-6 grid md:grid-cols-2 gap-3">
          <AbsenceReportButton matchId={matchId} reportedPlayerId={playerB.id} reporterId={currentUserId} />
          <ReportProblemButton matchId={matchId} reporterId={currentUserId} />
        </div>
        ...
        {/* 💬 CHAT PRIVÉ ADVERSAIRES - RLS stricte */}
        {canViewSalon && (
          <div className="mt-6">
            <MatchChat matchId={matchId} currentUserId={currentUserId} />
          </div>
        )}
        ...
        {statut === "TERMINE" && (
          <div className="mt-6">
            <RematchButton originalMatchId={matchId} requesterId={currentUserId} opponentId={currentUserId === "player_a" ? "player_b" : "player_a"} />
            <p className="mt-2 text-[10px] text-zinc-500">Réutilise système 1V1 existant, respecte paiement 500F, RLS, notifications. Adversaire reçoit 🔔 Raven veut une revanche [ACCEPTER][REFUSER] → nouveau match 1V1 auto</p>
          </div>
        )}
